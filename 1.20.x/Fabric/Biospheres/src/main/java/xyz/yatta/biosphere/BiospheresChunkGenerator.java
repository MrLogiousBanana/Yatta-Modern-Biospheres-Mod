...
public class BiospheresChunkGenerator extends ChunkGenerator {
	...
	private BlockState randomOreBlock(int x, int y, int z) {
		...
		java.util.Optional<net.minecraft.registry.entry.RegistryEntryList.Named<net.minecraft.block.Block>> oresC = net.minecraft.registry.Registries.BLOCK.getEntryList(net.minecraft.registry.tag.TagKey.of(net.minecraft.registry.RegistryKeys.BLOCK, new net.minecraft.util.Identifier("c", targetPath)));
	...
	private BlockState randomCaveOre(int y) {
		...
		if (y < 0) {
			if (rng < 0.2f) return net.minecraft.block.Blocks.DEEPSLATE_DIAMOND_ORE.getDefaultState();
			if (rng < 0.5f) return net.minecraft.block.Blocks.DEEPSLATE_REDSTONE_ORE.getDefaultState();
			if (rng < 0.8f) return net.minecraft.block.Blocks.DEEPSLATE_LAPIS_ORE.getDefaultState();
			return net.minecraft.block.Blocks.DEEPSLATE_GOLD_ORE.getDefaultState();
		} else {
			if (rng < 0.4f) return net.minecraft.block.Blocks.IRON_ORE.getDefaultState();
			if (rng < 0.8f) return net.minecraft.block.Blocks.COAL_ORE.getDefaultState();
			if (rng < 0.9f) return net.minecraft.block.Blocks.COPPER_ORE.getDefaultState();
			return net.minecraft.block.Blocks.GOLD_ORE.getDefaultState();
		}
	}
	...
	@Override
	public java.util.concurrent.CompletableFuture<net.minecraft.world.chunk.Chunk> populateBiomes(java.util.concurrent.Executor executor, net.minecraft.world.gen.noise.NoiseConfig noiseConfig, net.minecraft.world.gen.chunk.Blender blender, net.minecraft.world.gen.StructureAccessor structureAccessor, net.minecraft.world.chunk.Chunk chunk) {
		if (this.actualSeed == null) {
			long s = extractSeedFromStructureAccessor(structureAccessor);
			if (s == 0) {
				try {
					long rsSeed = noiseConfig.getOrCreateRandomDeriver(new net.minecraft.util.Identifier("biospheres", "seed")).split(0, 0, 0).nextLong();
	...
	public void generateFeatures(StructureWorldAccess world, Chunk chunk, StructureAccessor structureAccessor) {
		...
		ChunkPos chunkPos = chunk.getPos();
		BlockPos centerPos = this.getNearestCenterSphere(new BlockPos(chunkPos.getStartX(), 0, chunkPos.getStartZ()));
		...
		if (this.getBiomeSource() instanceof BiospheresBiomeSource) {
			...
			biome = chunk.getBiomeForNoiseGen(centerPos.getX() >> 2, 0, centerPos.getZ() >> 2);
		}

		if (isCaveBiome(biome)) {
			net.minecraft.util.math.random.Xoroshiro128PlusPlusRandom random = new net.minecraft.util.math.random.Xoroshiro128PlusPlusRandom(this.getActualSeed() ^ chunkPos.toLong());
			...
			for (int i = 0; i < 20; i++) {
				...
				int y = this.getMinimumY() + random.nextInt(this.getWorldHeight());

				if (centerPos.getSquaredDistance(new net.minecraft.util.math.Vec3i(x, y, z)) <= sRadius * sRadius) {
					BlockState oreState = getOreForHeight(y, random);
					if (oreState != null) {
						int veinSize = 3 + random.nextInt(5);
						net.minecraft.util.math.BlockPos.Mutable current = new net.minecraft.util.math.BlockPos.Mutable(x, y, z);
						for (int v = 0; v < veinSize; v++) {
							if (centerPos.getSquaredDistance(current) <= sRadius * sRadius) {
								BlockState state = chunk.getBlockState(current);
								if (state.isOf(net.minecraft.block.Blocks.STONE) || state.isOf(net.minecraft.block.Blocks.DEEPSLATE)) {
	...
	private BlockState getOreForHeight(int y, net.minecraft.util.math.random.Xoroshiro128PlusPlusRandom random) {
		if (y < 0) {
			int r = random.nextInt(100);
			if (r < 30) return net.minecraft.block.Blocks.DEEPSLATE_REDSTONE_ORE.getDefaultState();
			if (r < 50) return net.minecraft.block.Blocks.DEEPSLATE_DIAMOND_ORE.getDefaultState();
			if (r < 70) return net.minecraft.block.Blocks.DEEPSLATE_GOLD_ORE.getDefaultState();
			if (r < 85) return net.minecraft.block.Blocks.DEEPSLATE_IRON_ORE.getDefaultState();
			if (r < 95) return net.minecraft.block.Blocks.DEEPSLATE_LAPIS_ORE.getDefaultState();
		} else if (y < 64) {
			int r = random.nextInt(100);
			if (r < 40) return net.minecraft.block.Blocks.IRON_ORE.getDefaultState();
			if (r < 70) return net.minecraft.block.Blocks.COPPER_ORE.getDefaultState();
			if (r < 90) return net.minecraft.block.Blocks.COAL_ORE.getDefaultState();
			if (r < 95) return net.minecraft.block.Blocks.GOLD_ORE.getDefaultState();
		} else {
			int r = random.nextInt(100);
			if (r < 60) return net.minecraft.block.Blocks.COAL_ORE.getDefaultState();
			if (r < 90) return net.minecraft.block.Blocks.IRON_ORE.getDefaultState();
			if (r < 95) return net.minecraft.block.Blocks.EMERALD_ORE.getDefaultState();
		}
		return null;
	}
	...
	public void finishBiospheres(StructureWorldAccess world, Chunk chunk) {
		...
		for (java.util.Map.Entry<net.minecraft.world.gen.structure.Structure, it.unimi.dsi.fastutil.longs.LongSet> entry : chunk.getStructureReferences().entrySet()) {
			...
			net.minecraft.world.gen.GenerationStep.Feature step = getStructureStep(structure);
		...
		for (int x = chunkPos.getStartX(); x <= chunkPos.getEndX(); x++) {
			for (int z = chunkPos.getStartZ(); z <= chunkPos.getEndZ(); z++) {
				...
				if (isCaveBiome(biome)) {
					for (int y = this.getMinimumY(); y < centerPos.getY() + sRadius - 1; y++) {
						current.set(x, y, z);
						if (centerPos.getSquaredDistance(current) <= (sRadius - 1) * (sRadius - 1)) {
							BlockState state = chunk.getBlockState(current);
							if (state.isOf(Blocks.STONE) || state.isOf(Blocks.DEEPSLATE)) {
								if (this.chunkRandom.nextFloat() < 0.010f) {
									BlockState ore = randomCaveOre(y, state.isOf(Blocks.DEEPSLATE));
									chunk.setBlockState(current, ore, false);
									net.minecraft.util.math.BlockPos below = current.down();
									if (below.getY() >= this.getMinimumY()) {
										BlockState belowState = chunk.getBlockState(below);
										if (belowState.isOf(Blocks.STONE) || belowState.isOf(Blocks.DEEPSLATE)) {
						...
						if (newRadialDistance >= sRadius) {
							...
							if (y >= this.getMinimumY() && y < this.getWorldHeight() + this.getMinimumY()) {
								...
								if (!chunk.getBlockState(current).isAir()) {
									BlockState state = chunk.getBlockState(current);
									if (state.isOf(net.minecraft.block.Blocks.END_PORTAL_FRAME) || state.isOf(net.minecraft.block.Blocks.END_PORTAL)) continue;
							...
							if (isCaveBiome(biome)) {
								...
								if (currState.isOf(Blocks.STONE) || currState.isOf(Blocks.DEEPSLATE)) {
			...
			if (distXZSq <= ore.radius * ore.radius) {
				for (int y = ore.center.getY() - ore.radius; y <= ore.center.getY() + ore.radius; y++) {
					...
					if (distSq <= ore.radius * ore.radius) {
						...
						BlockState state;
						if (isSurface) {
							state = net.minecraft.block.Blocks.GLASS.getDefaultState();
						} else {
	...
	public void makeBridges(BlockPos pos, BlockPos centerPos, BlockPos[] nesw, net.minecraft.world.chunk.Chunk chunk, net.minecraft.util.math.BlockPos.Mutable current, double sRadius, OreSphere[] oreSpheres) {
	...
	public void fillBridgeSlice(BlockPos pos, BlockPos centerPos, Chunk chunk, BlockPos.Mutable current, boolean isOnXAxis, boolean isPositive, boolean clearOnly, boolean cn1, boolean cp1, boolean cn2, boolean cp2) {
		int x = pos.getX();
		int y = pos.getY();
		int z = pos.getZ();
		int cx = centerPos.getX();
				...
				if (isOnXAxis) {
					if (z == cz - 2) {
						...
						if (edge.getBlock() instanceof net.minecraft.block.FenceBlock) {
							edge = edge.with(net.minecraft.block.FenceBlock.EAST, cp1).with(net.minecraft.block.FenceBlock.WEST, cn1);
						}
						this.safeSetBlock(chunk, current, x, y, z, edge);
					} else if (z == cz + 2) {
						BlockState edge = this.defaultEdge;
						if (edge.getBlock() instanceof net.minecraft.block.FenceBlock) {
							edge = edge.with(net.minecraft.block.FenceBlock.EAST, cp2).with(net.minecraft.block.FenceBlock.WEST, cn2);
						}
						this.safeSetBlock(chunk, current, x, y, z, edge);
					}
				} else {
					if (x == cx - 2) {
						BlockState edge = this.defaultEdge;
						if (edge.getBlock() instanceof net.minecraft.block.FenceBlock) {
							edge = edge.with(net.minecraft.block.FenceBlock.SOUTH, cp1).with(net.minecraft.block.FenceBlock.NORTH, cn1);
						}
						this.safeSetBlock(chunk, current, x, y, z, edge);
					} else if (x == cx + 2) {
						BlockState edge = this.defaultEdge;
						if (edge.getBlock() instanceof net.minecraft.block.FenceBlock) {
							edge = edge.with(net.minecraft.block.FenceBlock.SOUTH, cp2).with(net.minecraft.block.FenceBlock.NORTH, cn2);
						}
						this.safeSetBlock(chunk, current, x, y, z, edge);
					}
				}
	...
	public void setStructureStarts(net.minecraft.registry.DynamicRegistryManager registryManager, net.minecraft.world.gen.chunk.placement.StructurePlacementCalculator placementCalculator, net.minecraft.world.gen.StructureAccessor structureAccessor, Chunk chunk, net.minecraft.structure.StructureTemplateManager structureTemplateManager) {
		...
		for (java.util.Map.Entry<net.minecraft.world.gen.structure.Structure, net.minecraft.structure.StructureStart> entry : starts.entrySet()) {
			...
			if (start != null && start.hasChildren()) {
				...
				if (step == net.minecraft.world.gen.GenerationStep.Feature.UNDERGROUND_STRUCTURES ||
					step == net.minecraft.world.gen.GenerationStep.Feature.STRONGHOLDS) {
					if (isVanilla && !isStronghold) {
						chunk.setStructureStart(structure, net.minecraft.structure.StructureStart.DEFAULT);
					}
				} else if (step == net.minecraft.world.gen.GenerationStep.Feature.SURFACE_STRUCTURES) {
		...
		int cxBase = Math.floorDiv(cx + this.sphereDistance / 2, this.sphereDistance) * this.sphereDistance;
		int czBase = Math.floorDiv(cz + this.sphereDistance / 2, this.sphereDistance) * this.sphereDistance;
		if (Math.floorDiv(cxBase, 16) == chunkPos.x && Math.floorDiv(czBase, 16) == chunkPos.z) {
			int cy = this.getNearestCenterSphere(new net.minecraft.util.math.BlockPos(cxBase, 0, czBase)).getY();
			net.minecraft.registry.entry.RegistryEntry<net.minecraft.world.biome.Biome> centerBiome = this.biomeSource.getBiome(cxBase / 4, cy / 4, czBase / 4, placementCalculator.getNoiseConfig().getMultiNoiseSampler());
			if (isCaveBiome(centerBiome)) {
				this.chunkRandom.setSeed(this.seed ^ (cxBase * 341873128712L) ^ (czBase * 132897987541L) ^ 543897123984L);
			}
		}
	}

	private net.minecraft.world.gen.GenerationStep.Feature getStructureStep(net.minecraft.world.gen.structure.Structure structure) {
		try {
			for (java.lang.reflect.Method m : net.minecraft.world.gen.structure.Structure.class.getMethods()) {
				if (m.getReturnType() == net.minecraft.world.gen.GenerationStep.Feature.class) {
					return (net.minecraft.world.gen.GenerationStep.Feature) m.invoke(structure);
				}
			}
		} catch (Exception e) {}
		return net.minecraft.world.gen.GenerationStep.Feature.SURFACE_STRUCTURES;
	}
	...
	private BlockState randomCaveOre(int y, boolean deepslate) {
		...
		if (y < 0) {
			if (r < 0.1f) return deepslate ? Blocks.DEEPSLATE_DIAMOND_ORE.getDefaultState() : Blocks.DIAMOND_ORE.getDefaultState();
			if (r < 0.3f) return deepslate ? Blocks.DEEPSLATE_GOLD_ORE.getDefaultState() : Blocks.GOLD_ORE.getDefaultState();
			if (r < 0.6f) return deepslate ? Blocks.DEEPSLATE_REDSTONE_ORE.getDefaultState() : Blocks.REDSTONE_ORE.getDefaultState();
			if (r < 0.8f) return deepslate ? Blocks.DEEPSLATE_LAPIS_ORE.getDefaultState() : Blocks.LAPIS_ORE.getDefaultState();
			return deepslate ? Blocks.DEEPSLATE_IRON_ORE.getDefaultState() : Blocks.IRON_ORE.getDefaultState();
		} else {
			if (r < 0.3f) return Blocks.COAL_ORE.getDefaultState();
			if (r < 0.6f) return Blocks.IRON_ORE.getDefaultState();
			if (r < 0.8f) return Blocks.COPPER_ORE.getDefaultState();
